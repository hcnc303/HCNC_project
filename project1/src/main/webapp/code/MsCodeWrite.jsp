...
<html>
	...
	<body>
	    ...
	    <div id="container">
	        <h2>대분류 코드 작성</h2>
			<form name="frm" method="post" action="MsCodeWriteSave.do">
				<table border="1" width="900px" height="150px">
					<tr>
						<th>대분류 코드</th>
						<td><input type="text" id= "masterCd" name="masterCd"></td>
					</tr>
					<tr>
						<th>대분류명</th>
						<td><input type="text" id= "masterNm" name="masterNm"></td>
					</tr>
					<tr>
						<th colspan="2">
	...
	<script>
		function fn_submit() {
			if(document.frm.masterCd.value != "" && document.frm.masterNm.value != "") {
				alert("저장되었습니다.")
			} else if(document.frm.masterCd.value == "") {
				alert("대분류 코드를 입력해주세요")
				document.frm.masterCd_focus();
				return false;
			} else if(document.frm.masterNm.value == ""){
				alert("대분류명를 입력해주세요")
				document.frm.masterNm_focus();
				return false;
			}
			document.frm.sumit();
		}
